...
def verifiziere_passwort(passwort, hashed_passwort):
    ...
    if BCRYPT_AVAILABLE:
        try:
            # Dekodiere Base64 zurück zu Bytes
        ...
        # Fallback: SHA256
        import hashlib
        return hashlib.sha256(passwort.encode()).hexdigest() == hashed_passwort
...
def daten_laden():
    ...
    if os.path.exists(daten_datei):
        with open(daten_datei, "r", encoding="utf-8") as f:
            ...
            benutzer_passwort = daten.get("benutzer_passwort", {"passwort": hash_passwort("Test1234")})
            
            # Migration: Falls Passwort noch nicht gehashed ist (alte Version), hashe es jetzt
            stored_pass = benutzer_passwort.get("passwort")
            if stored_pass and len(stored_pass) < 40:  # SHA256-Hashes sind 64 Zeichen, bcrypt+Base64 ist länger
                # Wahrscheinlich ein Klartext-Passwort oder altes Format
...
def passwort_login():
    ...
    """
    hashed_passwort = benutzer_passwort.get("passwort", hash_passwort("Test1234"))
    anmeldeversuche = 0
